...
> 메시징·구독·인증·리포트·후기 상태는 인메모리 스토어(`lib/store.ts`)로 전체 흐름을 즉시
> 체험할 수 있습니다. 환경변수를 채우면 동일 코드가 Supabase Auth/DB를 우선 사용합니다.

> E2E: 브라우저 시나리오 총 20건(Sprint 3: 구독 잠금/해제·인증 승인 반영 11건, Sprint 4:
> 리포트→대시보드·후기 인증 9건)으로 검증.
...
`docs/MVP.md` 7장 참고. Sprint 1~4로 **출시 최소선 + 락인** 골격 완성.
다음 **Sprint 5(확장)**: 결제 보호/분쟁 중재, 학부모 프리미엄 구독, 우선 매칭권, 모의 레테.
인프라: 인메모리 스토어(`lib/store.ts`) → Supabase `inquiries`/`messages`/`verifications`/
`lesson_reports`/`reviews` + Realtime 구독, 실결제(토스/카카오페이) 연동,
레테 D-day 알림톡/이메일 발송.
